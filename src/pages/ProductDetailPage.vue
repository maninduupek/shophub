<template>
  <div class="space-y-6">
    ...
    <div v-else-if="error" class="rounded-3xl border border-red-200 bg-red-50 p-10 text-center shadow-sm">
      <p class="text-red-700">{{ error }}</p>
    </div>

    <div v-else-if="product" class="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
      ...
      <div class="space-y-6">
        <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          ...
          <button
            ...
            class="w-full rounded-2xl bg-brand-600 px-6 py-3 text-lg font-semibold text-white transition-all duration-300 hover:bg-brand-700 disabled:bg-slate-300 disabled:cursor-not-allowed"
          >
